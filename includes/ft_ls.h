...
/*   Updated: 2019/06/21 18:56:14 by lpersin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				get_args(int ac, char **argv, t_options *const options,
						t_list **paths_lst);
...
void				recursive_ls(char *path, t_list *current_entries,
						t_options *const options);
t_list				*parse_user_args(t_list **head, t_options *options);
int					is_symlink(t_list *node);
void				l_print(t_list *node, int single);
void				ft_ls(t_list *paths_lst, t_options *options,
						int single_entry);
...
void				l_print_helper(t_entry *entry, t_list *head, char **str,
						int str_len);
